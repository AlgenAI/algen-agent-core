...
from traccia_runtime.context.builder import RetrievalContextBuilder
from traccia_runtime.models.providers.mock import MockModelProvider
from traccia_runtime.retrieval.contracts import RetrievalQuery, SourceDocument
from traccia_runtime.retrieval.memory import InMemoryRetriever
...
async def test_rag_agent_retrieves_verifies_and_returns_citations() -> None:
    ...
    provider = MockModelProvider(
        ["Employees receive twenty days of annual leave [S1]."]
    )
    ...
    runtime.contexts.register(
        RetrievalContextBuilder(
            "retrieval.knowledge", runtime.memory, retriever, minimum_score=0
        )
    )
