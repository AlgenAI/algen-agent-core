...
from examples.text_to_sql_agent.app import (
    AGENT_BY_PROVIDER,
    EXAMPLE_DIR,
...
from traccia_runtime.tools.contracts import ToolContext
...
async def test_text_to_sql_example_configuration_and_query() -> None:
    ...
    assert "orders.status = 'completed'" in agents[
        AGENT_BY_PROVIDER["mistral"]
    ].system_instructions
...
async def test_text_to_sql_closes_database_and_observability(
    ...
    monkeypatch.setattr(
        "examples.text_to_sql_agent.app.ReadOnlyAnalyticsDatabase", FakeDatabase
    )
    monkeypatch.setattr(
        "examples.text_to_sql_agent.app.create_query_tool", lambda database: object()
    )
    monkeypatch.setattr(
        "examples.text_to_sql_agent.app.build_container", lambda settings: FakeContainer()
    )
    monkeypatch.setattr("examples.text_to_sql_agent.app.TracciaRuntimeClient", FakeClient)
