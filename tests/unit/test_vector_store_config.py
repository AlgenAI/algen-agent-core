...
from traccia_runtime.config.settings import AppSettings, RetrievalSettings
from traccia_runtime.retrieval.contracts import RetrievalMode, RetrievalQuery, SourceDocument
from traccia_runtime.retrieval.memory import HashingEmbedder
from traccia_runtime.retrieval.vector_stores import ChromaRetriever, PgVectorRetriever
...
def test_all_vector_store_types_are_validated() -> None:
    ...
    assert RetrievalSettings(type="pgvector", connection_url="env://PGVECTOR_DSN").type == "pgvector"
...
class FakeChromaCollection:
    ...
    def get(self, *, where: dict[str, object]) -> dict[str, list[str]]:
        ...
        ids = [
            ...
            if all(metadata.get(key) == value for condition in conditions for key, value in condition.items())  # type: ignore[union-attr]
        ]
        return {"ids": ids}
    ...
    def query(self, **kwargs: object) -> dict[str, list[list[object]]]:
        ...
        selected = [
            ...
            if all(metadata.get(key) == value for condition in conditions for key, value in condition.items())
        ]
