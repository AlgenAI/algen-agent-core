...
async def test_end_to_end_direct_run(runtime) -> None:
    result = await runtime.run(RunRequest(
        agent="test-agent", input="question", tenant_id="tenant", user_id="user"
    ))
...
async def test_tenant_cannot_read_another_run(runtime) -> None:
    state = await runtime.start(RunRequest(
        agent="test-agent", input="question", tenant_id="tenant-a", user_id="user"
    ))
