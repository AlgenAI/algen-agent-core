...
async def test_timeout_is_terminal() -> None:
    ...
    result = await runtime.run(RunRequest(
        agent="test-agent", input="x", tenant_id="t", user_id="u", timeout_seconds=0.01
    ))
...
async def test_cancellation_is_terminal() -> None:
    ...
    state = await runtime.start(RunRequest(agent="test-agent", input="x", tenant_id="t", user_id="u"))
...
async def test_streaming_emits_deltas() -> None:
    ...
    result = await runtime.run(RunRequest(
        agent="test-agent", input="x", tenant_id="t", user_id="u", stream=True
    ))
