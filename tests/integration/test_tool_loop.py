...
async def test_model_tool_model_loop() -> None:
    ...
    runtime.tools.register(Tool(
        ToolDefinition(
            name="math.add", version="1", description="add",
    ...
    result = await runtime.run(RunRequest(agent="test-agent", input="2+3", tenant_id="t", user_id="u"))
...
async def test_side_effect_approval_pause_resume() -> None:
    runtime = make_runtime(
        ...
        make_agent(enabled_tools=frozenset({"external.write"}), tool_permissions=frozenset({"write"})),
    )
    calls = 0
    async def handler(args, ctx):
        ...
        calls += 1
        return {"ok": True}
    runtime.tools.register(Tool(ToolDefinition(
        name="external.write", version="1", description="write", required_permissions=frozenset({"write"}),
        side_effect=SideEffect.WRITE,
        input_schema={"type": "object", "properties": {"value": {"type": "string"}}, "required": ["value"]},
        output_schema={"type": "object", "properties": {"ok": {"type": "boolean"}}, "required": ["ok"]},
    ), handler))
    paused = await runtime.run(RunRequest(agent="test-agent", input="write", tenant_id="t", user_id="u"))
