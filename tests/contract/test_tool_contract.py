...
def context() -> ToolContext:
    return ToolContext(
        run_id="r", step_id="s", tenant_id="t", user_id="u",
        permissions=frozenset(), idempotency_key="key",
    )
...
async def test_tool_input_output_and_idempotency_contract() -> None:
    ...
    registry.register(Tool(ToolDefinition(
        name="test.increment", version="1.0.0", description="increment",
...
async def test_invalid_tool_input_is_bounded_error() -> None:
    registry = ToolRegistry()
    registry.register(Tool(ToolDefinition(
        name="test.input", version="1", description="test",
        input_schema={"type": "object", "required": ["x"]}, output_schema={},
