...
class InMemoryConversationEventBus:
    ...
    async def history(
        ...
        async with self._lock:
            return tuple(
                event
                for event in self._history.get(conversation_id, ())
...
class PostgresConversationStore:
    ...
    async def create(self, conversation: Conversation) -> None:
        await self._database.execute(
            "INSERT INTO traccia_runtime_conversations "
    ...
    async def get(self, conversation_id: str, tenant_id: str) -> Conversation | None:
        row = await self._database.fetchrow(
            "SELECT conversation FROM traccia_runtime_conversations WHERE id=$1 AND tenant_id=$2",
            conversation_id,
            tenant_id,
        )
    ...
    async def list(
        ...
        rows = await self._database.fetch(
            "SELECT conversation FROM traccia_runtime_conversations "
    ...
    async def save(self, conversation: Conversation, expected_version: int) -> None:
        ...
        result = await self._database.execute(
            "UPDATE traccia_runtime_conversations SET version=$1, conversation=$2::jsonb, "
    ...
    async def append_message(self, message: ConversationMessage) -> None:
        await self._database.execute(
            "INSERT INTO traccia_runtime_conversation_messages "
    ...
    async def update_message(self, message: ConversationMessage) -> None:
        result = await self._database.execute(
            "UPDATE traccia_runtime_conversation_messages SET message=$1::jsonb "
    ...
    async def messages(
        ...
        rows = await self._database.fetch(
            "SELECT message FROM traccia_runtime_conversation_messages "
        ...
        return tuple(
            ConversationMessage.model_validate_json(row["message"])
...
class PostgresConversationEventBus:
    ...
    async def next_sequence(self, conversation_id: str) -> int:
        row = await self._database.fetchrow(
            ...
            "FROM traccia_runtime_conversation_events WHERE conversation_id=$1",
            conversation_id,
        )
    ...
    async def publish(self, event: ConversationEvent) -> None:
        await self._database.execute(
            "INSERT INTO traccia_runtime_conversation_events "
    ...
    async def history(
        ...
        rows = await self._database.fetch(
            "SELECT event FROM traccia_runtime_conversation_events "
