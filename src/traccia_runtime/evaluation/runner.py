...
from traccia_runtime.evaluation.contracts import (
    CaseEvaluation,
    EvaluationArtifact,
    EvaluationReport,
    GoldenQuestion,
...
class EvaluationRunner:
    def __init__(self) -> None:
        self._tracer = trace.get_tracer("traccia_runtime.evaluation")
        self._counter = metrics.get_meter("traccia_runtime.evaluation").create_counter(
            "traccia_runtime.evaluation.cases"
        )
    ...
    async def run(
        ...
        *,
        ...
        return EvaluationReport(
            cases=tuple(results),
            score=score,
            promoted=bool(results) and score >= promotion_threshold and all(item.passed for item in results),
