...
async def evaluate_policy_observed(
    *,
    ...
    span_attributes.update(
        {
            ...
            "policy.source_sdk": "traccia_runtime",
    ...
    with tracer.start_as_current_span(
        ...
        if invoked:
            span.set_attribute(
                "policy.policies_invoked", tuple(str(item) for item in invoked)
            )
        if triggered:
            span.set_attribute(
                "policy.policies_triggered", tuple(str(item) for item in triggered)
            )
        if skipped:
            span.set_attribute(
                "policy.policies_skipped", tuple(str(item) for item in skipped)
            )
        return decision
