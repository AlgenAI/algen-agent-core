...
from traccia_runtime.policies.contracts import (
    PolicyAction,
    PolicyDecision,
    PolicyPoint,
)
...
class PIIRedactionPolicy:
    ...
    async def evaluate(self, point: str, payload: Any, context: Mapping[str, Any]) -> PolicyDecision:
        ...
        return PolicyDecision(action=PolicyAction.ALLOW, reason_code="pii.clear", reason="No PII detected.")
...
class PromptInjectionPolicy:
    ...
    _patterns = (
        ...
        re.compile(r"(?i)\b(?:reveal|print|show|repeat|expose)\b.{0,40}\b(?:system|developer)\s+(?:prompt|message|instructions?)\b"),
        re.compile(r"(?i)\b(?:bypass|disable|override)\b.{0,40}\b(?:guardrails?|safety|policy|instructions?)\b"),
...
class CompositePolicyEngine:
    def __init__(
        ...
        self._tracer = trace.get_tracer("traccia_runtime.guardrails")
    ...
    async def evaluate(self, point: str, payload: Any, context: Mapping[str, Any]) -> PolicyDecision:
        try:
            ...
            raise ValueError(f"unknown policy point {point!r}; expected one of: {expected}") from exc
        transformed = payload
        transforms: list[str] = []
