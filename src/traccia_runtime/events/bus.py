...
from traccia_runtime.events.contracts import AuditEvent, RunEvent
...
class PostgresEventBus:
    ...
    async def next_sequence(self, run_id: str) -> int:
        row = await self._database.fetchrow(
            ...
            "FROM traccia_runtime_events WHERE run_id=$1",
            run_id,
        )
    ...
    async def publish(self, event: RunEvent) -> None:
        await self._database.execute(
            "INSERT INTO traccia_runtime_events "
    ...
    async def history(self, run_id: str, after: int = 0) -> tuple[RunEvent, ...]:
        rows = await self._database.fetch(
            "SELECT event FROM traccia_runtime_events "
...
class PostgresAuditLog:
    ...
    async def append(self, event: AuditEvent) -> None:
        await self._database.execute(
            "INSERT INTO traccia_runtime_audit_events "
    ...
    async def list(
        self, tenant_id: str, resource_id: str | None = None
    ) -> tuple[AuditEvent, ...]:
        rows = await self._database.fetch(
            "SELECT event FROM traccia_runtime_audit_events WHERE tenant_id=$1 "
