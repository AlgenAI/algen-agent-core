...
class LocalTransformersProvider:
    ...
    async def _load(self) -> Any:
        if self._pipeline is None:
            try:
                ...
                raise CapabilityError("install traccia-runtime[transformers] for local Transformers") from exc
    ...
    async def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        return ModelResponse(message=Message.text(Role.ASSISTANT, text), model=self._model, provider=self.provider_id)
