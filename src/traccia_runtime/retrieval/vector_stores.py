...
from traccia_runtime.retrieval.contracts import (
    RetrievalMode,
    RetrievalQuery,
    RetrievedDocument,
    SourceDocument,
...
from traccia_runtime.retrieval.memory import Embedder
...
class PgVectorRetriever(_VectorStoreBase):
    ...
    def __init__(
        ...
        *,
        dimensions: int,
        table: str = "traccia_runtime_documents",
        ...
        async with self._ready_lock:
            ...
            if self._pool is None:
                try:
                    ...
                    raise ImportError("install algen-traccia-runtime[pgvector]") from exc
    ...
    async def ingest(self, documents: Sequence[SourceDocument]) -> int:
        ...
        owners = {(str(chunk.metadata.get("tenant_id", "default")), chunk.document_id) for chunk in chunks}
        async with self._pool.acquire() as connection:
            async with connection.transaction():
                ...
                await connection.executemany(
                    ...
                    [
                        (
                            str(chunk.metadata.get("tenant_id", "default")), chunk.id,
                            chunk.document_id or chunk.id, chunk.text, chunk.source, chunk.title,
                            chunk.uri, chunk.chunk_index, json.dumps(chunk.metadata),
                            self._vector_factory(vector),
                        )
    ...
    async def retrieve(self, query: RetrievalQuery) -> tuple[RetrievedDocument, ...]:
        ...
        parameters: list[Any] = [query.tenant_id, json.dumps(query.filters), vector, query.text, query.limit]
        score = (
...
class ChromaRetriever(_VectorStoreBase):
    def __init__(
        ...
        embedder: Embedder,
        *,
        collection_name: str = "traccia_runtime_documents",
        path: str | None = None,
        ...
        if client is None:
            try:
                ...
                raise ImportError("install algen-traccia-runtime[chroma]") from exc
...
class FaissRetriever(_VectorStoreBase):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ImportError("install algen-traccia-runtime[faiss]") from exc
    ...
    async def ingest(self, documents: Sequence[SourceDocument]) -> int:
        ...
        async with self._lock:
            owners = {(str(item.metadata.get("tenant_id", "default")), item.id) for item in documents}
            retained = []
    ...
    async def delete(self, tenant_id: str, document_id: str) -> int:
        async with self._lock:
            ...
            self._records = [record for record in self._records if not (record["tenant_id"] == tenant_id and record["document_id"] == document_id)]
    ...
    async def retrieve(self, query: RetrievalQuery) -> tuple[RetrievedDocument, ...]:
        ...
        for score, index in zip(scores[0], indices[0], strict=True):
            ...
            if record["tenant_id"] != query.tenant_id:
                continue
            if any(record.get("metadata", {}).get(key) != value for key, value in query.filters.items()):
                continue
...
class QdrantRetriever(_VectorStoreBase):
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ImportError("install algen-traccia-runtime[qdrant]") from exc
        self._models = models
    ...
    def _filter(self, tenant_id: str, filters: Mapping[str, Any] | None = None) -> Any:
        conditions = [self._models.FieldCondition(key="tenant_id", match=self._models.MatchValue(value=tenant_id))]
        conditions.extend(
            ...
            await self._client.upsert(
                ...
                points=[
                    self._models.PointStruct(
                        id=str(uuid5(_QDRANT_NAMESPACE, f"{chunk.metadata.get('tenant_id', 'default')}:{chunk.id}")),
                        vector=vector,
    ...
    async def delete(self, tenant_id: str, document_id: str) -> int:
        ...
        points, _ = await self._client.scroll(
            ...
            scroll_filter=self._models.Filter(
                must=[
                    self._models.FieldCondition(key="tenant_id", match=self._models.MatchValue(value=tenant_id)),
                    self._models.FieldCondition(key="document_id", match=self._models.MatchValue(value=document_id)),
                ]
            ),
            limit=10_000,
            with_payload=False,
    ...
    async def retrieve(self, query: RetrievalQuery) -> tuple[RetrievedDocument, ...]:
        ...
        return tuple(_from_payload(point.payload or {}, float(point.score)) for point in response.points)
