...
class ModelProviderEmbedder:
    def __init__(
        self, provider: ModelEmbeddingProvider, model: str | None = None
    ) -> None:
...
class InMemoryRetriever:
    ...
    async def ingest(self, documents: Sequence[SourceDocument]) -> int:
        ...
        async with self._lock:
            owners = {
                (chunk.document_id, chunk.metadata.get("tenant_id")) for chunk in chunks
            }
            stale_ids = {
                item.id
    ...
    async def delete(self, tenant_id: str, document_id: str) -> int:
        async with self._lock:
            pending_count = sum(
                ...
                and item.metadata.get("tenant_id") == tenant_id
            )
            self._pending = [
                item
                for item in self._pending
    ...
    async def retrieve(self, query: RetrievalQuery) -> tuple[RetrievedDocument, ...]:
        ...
        for document in self._documents:
            ...
            vector_score = max(
                0.0, self._cosine(query_vector, self._vectors.get(document.id, []))
            )
