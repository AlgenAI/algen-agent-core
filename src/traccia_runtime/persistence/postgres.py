...
from traccia_runtime.types.contracts import Artifact, Message, RunState
...
class PostgresDatabase:
    ...
    async def start(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                raise ImportError("install traccia-runtime[postgres]") from exc
    ...
    async def _migrate(self, pool: Any) -> None:
        await pool.execute(
            "CREATE TABLE IF NOT EXISTS traccia_runtime_schema_migrations "
            "(name text PRIMARY KEY, applied_at timestamptz NOT NULL DEFAULT now())"
        )
        rows = await pool.fetch("SELECT name FROM traccia_runtime_schema_migrations")
        ...
        for migration in sorted(directory.glob("*.sql")):
            ...
            async with pool.acquire() as connection, connection.transaction():
                ...
                await connection.execute(
                    "INSERT INTO traccia_runtime_schema_migrations (name) VALUES ($1)",
                    migration.name,
                )
...
class PostgresRunStore:
    ...
    async def create(self, state: RunState) -> None:
        await self._database.execute(
            "INSERT INTO traccia_runtime_runs (id, tenant_id, version, state) "
    ...
    async def get(self, run_id: str, tenant_id: str) -> RunState | None:
        row = await self._database.fetchrow(
            "SELECT state FROM traccia_runtime_runs WHERE id=$1 AND tenant_id=$2",
            run_id,
            tenant_id,
        )
    ...
    async def save(self, state: RunState, expected_version: int) -> None:
        ...
        result = await self._database.execute(
            "UPDATE traccia_runtime_runs SET version=$1, state=$2::jsonb, updated_at=now() "
    ...
    async def list_active(self, limit: int = 1000) -> Sequence[RunState]:
        rows = await self._database.fetch(
            "SELECT state FROM traccia_runtime_runs "
...
class PostgresMemoryStore:
    ...
    async def append(
        ...
        for message in messages:
            await self._database.execute(
                "INSERT INTO traccia_runtime_memory "
    ...
    async def get(self, tenant_id: str, session_id: str, limit: int) -> Sequence[Message]:
        rows = await self._database.fetch(
            "SELECT message FROM traccia_runtime_memory "
    ...
    async def delete(self, tenant_id: str, session_id: str) -> None:
        await self._database.execute(
            "DELETE FROM traccia_runtime_memory WHERE tenant_id=$1 AND session_id=$2",
            tenant_id,
            session_id,
        )
...
class PostgresArtifactStore:
    ...
    async def put(self, artifact: Artifact) -> None:
        ...
        await self._database.execute(
            "INSERT INTO traccia_runtime_artifacts "
    ...
    async def get(self, artifact_id: str, tenant_id: str) -> Artifact | None:
        row = await self._database.fetchrow(
            ...
            "FROM traccia_runtime_artifacts WHERE id=$1 AND tenant_id=$2",
            artifact_id,
            tenant_id,
        )
...
class PostgresToolExecutionStore:
    ...
    async def begin(
        ...
        result = await self._database.execute(
            "INSERT INTO traccia_runtime_tool_executions "
        ...
        if result == "INSERT 0 1":
            return record, True
        row = await self._database.fetchrow(
            "SELECT record FROM traccia_runtime_tool_executions "
        ...
        if current.status == ToolExecutionStatus.FAILED:
            changed = await self._database.execute(
                "UPDATE traccia_runtime_tool_executions SET status='started', record=$1::jsonb, "
    ...
    async def _get(self, idempotency_key: str) -> ToolExecutionRecord:
        row = await self._database.fetchrow(
            "SELECT record FROM traccia_runtime_tool_executions WHERE idempotency_key=$1",
            idempotency_key,
        )
        if row is None:
    ...
    async def _update(self, record: ToolExecutionRecord) -> None:
        await self._database.execute(
            "UPDATE traccia_runtime_tool_executions SET status=$1, record=$2::jsonb, "
