...
from traccia_runtime.frameworks.contracts import (
...
from traccia_runtime.types.contracts import TokenUsage
...
class LangGraphAdapter(_BaseAdapter):
    ...
    async def invoke(self, request: FrameworkRunRequest) -> FrameworkRunResult:
        ...
        return FrameworkRunResult(
            ...
            status=(FrameworkRunStatus.AWAITING_INPUT if interruptions else FrameworkRunStatus.COMPLETED),
...
class OpenAIAgentsAdapter(_BaseAdapter):
    ...
    def __init__(self, agent: Any, *, runner: Any | None = None, run_config: Any = None) -> None:
        ...
        if runner is None:
            try:
                ...
                raise ImportError("install algen-traccia-runtime[openai-agents]") from exc
            runner = Runner
        self._agent = agent
        self._runner = runner
    ...
    async def invoke(self, request: FrameworkRunRequest) -> FrameworkRunResult:
        ...
        return FrameworkRunResult(
            ...
            resume_state=result.to_state() if interruptions and hasattr(result, "to_state") else None,
        )
...
class AutoGenAdapter(_BaseAdapter):
    ...
    async def invoke(self, request: FrameworkRunRequest) -> FrameworkRunResult:
        ...
        return FrameworkRunResult(
            output=_text(final), usage=usage,
