...
from traccia_runtime.exceptions.errors import ConfigurationError, NotFoundError, ProviderError
...
class ModelServiceRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._tracer = trace.get_tracer("traccia_runtime.model_services")
        self._counter = metrics.get_meter("traccia_runtime.model_services").create_counter(
            "traccia_runtime.model_service.calls"
        )
        ...
        *,
        ...
        for name, version in candidates:
            try:
                ...
                with self._tracer.start_as_current_span("model_service.invoke", attributes=attributes):
    ...
    def _resolve(self, name: str, version: str | None) -> ModelService:
        candidates = [
            ...
            if candidate_name == name and (version is None or candidate_version == version)
