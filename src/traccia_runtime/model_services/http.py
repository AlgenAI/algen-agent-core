...
from traccia_runtime.security.network import validate_outbound_url
...
class HTTPModelService:
    ...
    async def predict(self, request: ModelServiceRequest) -> ModelServiceResponse:
        ...
        async with httpx.AsyncClient(timeout=request.timeout_seconds or self.manifest.timeout_seconds) as client:
    ...
    async def health(self) -> ModelServiceHealth:
        ...
        try:
            await validate_outbound_url(
                self._health_endpoint, allowed_hosts=self._allowed_hosts
            )
            ...
            return ModelServiceHealth(
                ...
                drift_state=self.manifest.drift_state,
                message=None if response.is_success else f"health returned HTTP {response.status_code}",
            )
