...
async def join_results(
    ...
    if relationship in {"one_to_one", "many_to_one"} and any(len(rows) > 1 for rows in index.values()):
    ...
    for row in left:
        ...
        if not matches:
            ...
            for match in matches:
                output.append({**row, **{key: value for key, value in match.items() if key not in keys}})
