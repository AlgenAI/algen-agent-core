...
from traccia_runtime.analytics.contracts import AnalyticalGraphState
...
class PostgresAnalyticalGraphStore:
    ...
    async def create(self, state: AnalyticalGraphState) -> None:
        await self._database.execute(
            "INSERT INTO traccia_runtime_analytical_graphs "
    ...
    async def get(self, graph_id: str, tenant_id: str) -> AnalyticalGraphState | None:
        row = await self._database.fetchrow(
            "SELECT state FROM traccia_runtime_analytical_graphs WHERE id=$1 AND tenant_id=$2",
            graph_id,
            tenant_id,
        )
    ...
    async def save(self, state: AnalyticalGraphState, expected_version: int) -> None:
        ...
        result = await self._database.execute(
            "UPDATE traccia_runtime_analytical_graphs SET status=$1, version=$2, "
