...
from traccia_runtime.exceptions.errors import ConflictError, NotFoundError
...
class PostgresApprovalService:
    ...
    async def create(
        ...
        await self._database.execute(
            "INSERT INTO traccia_runtime_approvals "
    ...
    async def get(self, approval_id: str, tenant_id: str) -> ApprovalRequest | None:
        row = await self._database.fetchrow(
            "SELECT approval FROM traccia_runtime_approvals WHERE id=$1 AND tenant_id=$2",
            approval_id,
            tenant_id,
        )
    ...
    async def _save(self, item: ApprovalRequest, expected: ApprovalStatus) -> bool:
        result = await self._database.execute(
            "UPDATE traccia_runtime_approvals SET approval=$1::jsonb, updated_at=now() "
