...
from traccia_runtime.distributed.contracts import WorkItem, WorkStatus
from traccia_runtime.exceptions.errors import ConflictError
from traccia_runtime.types.contracts import utc_now
...
class PostgresWorkQueue:
    ...
    async def enqueue(self, item: WorkItem) -> tuple[WorkItem, bool]:
        result = await self._database.execute(
            "INSERT INTO traccia_runtime_work_items "
            ...
            item.id, item.tenant_id, item.kind, item.deduplication_key, item.status.value,
            item.attempts, item.maximum_attempts, item.available_at, item.model_dump_json(),
        )
        if result == "INSERT 0 1":
            return item, True
        row = await self._database.fetchrow(
            "SELECT item FROM traccia_runtime_work_items WHERE tenant_id=$1 AND deduplication_key=$2",
    ...
    async def claim(self, worker_id: str, lease_seconds: int) -> WorkItem | None:
        row = await self._database.fetchrow(
            "WITH candidate AS (SELECT id FROM traccia_runtime_work_items "
            ...
            "UPDATE traccia_runtime_work_items w SET status='leased',lease_owner=$1,"
    ...
    async def renew(self, item_id: str, worker_id: str, lease_seconds: int) -> bool:
        result = await self._database.execute(
            "UPDATE traccia_runtime_work_items SET lease_expires_at=now()+($1*interval '1 second'),"
    ...
    async def complete(self, item_id: str, worker_id: str, result: dict[str, object]) -> None:
        updated = await self._database.execute(
            "UPDATE traccia_runtime_work_items SET status='completed',result=$1::jsonb,"
            ...
            __import__("json").dumps(result), item_id, worker_id,
        )
        if updated != "UPDATE 1":
            raise ConflictError("work item lease is not owned by this worker")
    ...
    async def fail(self, item_id: str, worker_id: str, error: str, retry: bool) -> None:
        ...
        updated = await self._database.execute(
            "UPDATE traccia_runtime_work_items SET "
            ...
            status, error, item_id, worker_id,
        )
        if updated != "UPDATE 1":
            raise ConflictError("work item lease is not owned by this worker")
