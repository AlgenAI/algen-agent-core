...
from traccia_runtime.analytics import AnalyticalResult, GraphExecutionContext
...
from traccia_runtime.semantics import AnalysisOutcome
...
class AnalyticalMethodRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._tracer = trace.get_tracer("traccia_runtime.methods")
        self._counter = metrics.get_meter("traccia_runtime.methods").create_counter(
            "traccia_runtime.method.executions"
        )
    ...
    def discover(self, *, include_deprecated: bool = False) -> tuple[AnalyticalMethodManifest, ...]:
        return tuple(
            manifest
            for manifest, _ in sorted(self._methods.values(), key=lambda item: (item[0].name, item[0].version))
            if include_deprecated or manifest.lifecycle != MethodLifecycle.DEPRECATED
        )

    def resolve(self, name: str, version: str | None = None) -> tuple[AnalyticalMethodManifest, AnalyticalMethod]:
        ...
        candidates = [item for item in candidates if item[0].lifecycle != MethodLifecycle.DEPRECATED]
        if not candidates:
            raise NotFoundError(f"analytical method {name!r} version {version or 'latest'!r} not found")
    ...
    async def execute(
        ...
        if manifest.required_history_points and sum(
            item.row_count or 0 for item in request.inputs
