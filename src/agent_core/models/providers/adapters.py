...
class OpenAIProvider(OpenAICompatibleProvider):
    def __init__(self, api_key_reference: str, default_model: str, **kwargs: Any) -> None:
        super().__init__("openai", "https://api.openai.com/v1", api_key_reference, default_model,
            ModelCapabilities(streaming=True, tools=True, structured_output=True, json_schema=True,
                              embeddings=True, images=True, files=True), **kwargs)


class AzureOpenAIProvider(OpenAICompatibleProvider):
    def __init__(self, base_url: str, api_key_reference: str, deployment: str, api_version: str,
                 secret_provider: Any | None = None, client: httpx.AsyncClient | None = None) -> None:
        super().__init__("azure_openai", base_url, None, deployment,
            ModelCapabilities(streaming=True, tools=True, structured_output=True, json_schema=True,
                              embeddings=True, images=True), secret_provider, client,
...
class AnthropicProvider(OpenAICompatibleProvider):
    ...
    def __init__(self, api_key_reference: str, default_model: str, base_url: str = "https://api.anthropic.com/v1",
                 **kwargs: Any) -> None:
        super().__init__("anthropic", base_url, api_key_reference, default_model,
            ModelCapabilities(streaming=True, tools=True, structured_output=False, json_schema=False, images=True),
            endpoint="/messages", **kwargs)
            ...
            async with self._client.stream(
                ...
                if response.is_error:
                    ...
                    raise ProviderError(
                        ...
                        ErrorKind.RATE_LIMIT if response.status_code == 429 else ErrorKind.UNAVAILABLE,
                        retryable,
                    )
...
class DeepSeekProvider(OpenAICompatibleProvider):
    def __init__(self, api_key_reference: str, default_model: str = "deepseek-chat", **kwargs: Any) -> None:
        super().__init__("deepseek", "https://api.deepseek.com/v1", api_key_reference, default_model,
            ModelCapabilities(streaming=True, tools=True, structured_output=True, json_schema=False), **kwargs)


class OllamaProvider(OpenAICompatibleProvider):
    def __init__(self, default_model: str, base_url: str = "http://127.0.0.1:11434/v1", **kwargs: Any) -> None:
        super().__init__("ollama", base_url, None, default_model,
            ModelCapabilities(streaming=True, tools=True, structured_output=True, json_schema=True,
                              embeddings=True, images=True), **kwargs)


class HuggingFaceInferenceProvider(OpenAICompatibleProvider):
    def __init__(self, api_key_reference: str, default_model: str,
                 base_url: str = "https://router.huggingface.co/v1", **kwargs: Any) -> None:
        super().__init__("huggingface_inference", base_url, api_key_reference, default_model,
            ModelCapabilities(streaming=True, tools=True, structured_output=False, json_schema=False), **kwargs)
