...
Traccia Runtime can integrate agents implemented with another orchestration framework. Adapters normalize
...
```bash
pip install 'algen-traccia-runtime[langgraph]'
pip install 'algen-traccia-runtime[openai-agents]'
pip install 'algen-traccia-runtime[autogen]'
pip install 'algen-traccia-runtime[crewai]'
```

`algen-traccia-runtime[frameworks]` installs all four. Core imports work when none are installed.
...
```python
from traccia_runtime.frameworks import FrameworkAdapterRegistry, FrameworkRunRequest, LangGraphAdapter
...
Traccia Runtime propagates run, session, tenant, and user identifiers, but an external framework remains
responsible for honoring them inside its own persistence and tools.

These are orchestration-framework adapters, not model-provider adapters. Native Traccia Runtime agents use
