...
Traccia Runtime is a restart-aware development runtime. It now has configurable
...
2. **Harden policy semantics.** Support approval and clarification outcomes at every declared policy
   boundary, signed policy bundles, explicit fail-open/fail-closed behavior, and adversarial tests for
...
6. **Strengthen readiness and lifecycle.** Probe every mandatory store, model and telemetry dependency;
   expire paused runs without user traffic; add health degradation reasons; and test repeated
   start/drain cycles.
...
- Object storage for large artifacts instead of PostgreSQL byte payloads.
