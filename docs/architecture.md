...
Traccia Runtime exposes a generic durable conversation layer above individual runs. A conversation
...
Built-in defaults are merged with YAML files, `TRACCIA_RUNTIME__` nested environment variables, and deployment overrides. Agent and request settings are resolved by orchestration; request overrides are strict and should be allowlisted by deployments. Unknown fields fail validation.
